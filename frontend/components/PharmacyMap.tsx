...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, SafeAreaView, TextInput, ScrollView, Platform, Image } from 'react-native';
import { MaterialIcons } from '@expo/vector-icons';
import { router } from 'expo-router';

// --- 1. CONDITIONAL IMPORT FOR MAPS ---
// We use 'require' inside a condition so Web doesn't crash trying to load native maps.
let MapView: any;
let Marker: any;
...
export default function MapPage() {
  ...
  if (Platform.OS === 'web') {
    return (
      <View style={styles.webContainer}>
        ...
        <TouchableOpacity onPress={() => router.push('/(tabs)/search')} style={styles.webButton}>
          <Text style={styles.webButtonText}>Browse List View</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
            ...
            >
              <View style={styles.markerWrapper}>
                ...
                <View style={[styles.markerBubble, isSelected && styles.markerBubbleSelected]}>
                  {pharmacy.hasDeal ? (
                     <MaterialIcons name="savings" size={18} color={isSelected ? "white" : "#fff"} />
                  ) : (
                     <MaterialIcons name="local-pharmacy" size={18} color={isSelected ? "white" : "#13ec80"} />
                  )}
                </View>
                <View style={styles.markerLabelContainer}>
                   <Text style={[styles.markerLabel, pharmacy.hasDeal && { color: '#d97706' }]}>
      ...
      {/* FLOATING BUTTONS */}
      <View style={[styles.floatingButtons, selectedId ? { bottom: 260 } : { bottom: 100 }]}>
...
const styles = StyleSheet.create({
  ...
  // Web Fallback Styles
  webContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f6f8f7' },
  webTitle: { fontSize: 24, fontWeight: 'bold', color: '#111814', marginTop: 16 },
  webSubtitle: { fontSize: 16, color: '#6b7280', marginTop: 8, marginBottom: 24 },
  webButton: { backgroundColor: '#13ec80', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12 },
  webButtonText: { color: '#111814', fontWeight: 'bold' },

  // Marker Styles
  markerWrapper: { alignItems: 'center', justifyContent: 'center' },
  markerPulse: { position: 'absolute', width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(19, 236, 128, 0.3)' },
  ...
  topContent: { paddingHorizontal: 16, paddingTop: 16 },
  ...
  chip: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', paddingHorizontal: 16, height: 36, borderRadius: 18, marginRight: 8, gap: 6, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2, marginBottom: 8 },
